...
# реализация неявного метода конечных разностей с использованием алгоритма прогонки
@jit(nopython=True)
def calculate_heat(Tl, Tr, L, h, total_time, dt, rho, c, lmd):
    Nx = int(L / h) #  Считаем, сколько "отрезков" поместится в длину пластины L
    Nt = int(total_time / dt) # Считаем, сколько "шагов" нужно сделать по времени

    T = np.zeros(Nx + 1) # Начальная температура 0
    T[0], T[-1] = Tl, Tr # Граничные условия:  Задаем неизменную температуру на левом и правом краях

    # Коэффициенты матрицы
    A = lmd / h**2 # Коэффициент влияния "левого" соседа
    C = A # Коэффициент влияния "правого" соседа (в симметричной задаче они равны)
    B = 2 * lmd / h**2 + rho * c / dt # Коэффициент самой точки (учитывает и теплопроводность, и инерцию материала)

    alpha = np.zeros(Nx + 1) # Массив для хранения вспомогательных коэффициентов связи
    beta = np.zeros(Nx + 1)
    ...
    for _ in range(Nt):
        ...
        for i in range(1, Nx):
            ...
            denom = B - C * alpha[i - 1]
            alpha[i] = A / denom
            beta[i] = (C * beta[i - 1] - Fi) / denom #  Вычисляем промежуточное значение температуры

        # Обратный ход
        T_new = np.zeros(Nx + 1)
        T_new[-1] = Tr
        for i in range(Nx - 1, 0, -1):
            T_new[i] = alpha[i] * T_new[i + 1] + beta[i] # # Вычисляем температуру через соседа справа
        
        T = T_new # Теперь "будущее" стало "настоящим". Переходим к следующему шагу времени.

    x_axis = np.linspace(0, L, Nx + 1)
    return x_axis, T, T[Nx // 2] # Возвращаем координаты, все температуры и температуру ровно в центре
...
class HeatApp:
    def __init__(self, root):
        ...
        self.btn_task = ttk.Button(side_panel, text="Выполнить задание (2 сек)", command=self.run_experiment)
    ...
    def run_experiment(self):
        try:
            ...
            for dt in dts:
                for h in hs:
                    # Запускаем расчет (обязательно передаем физические параметры)
